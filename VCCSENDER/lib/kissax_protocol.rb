...
module Cosmos
    class KissaxProtocol < Protocol
        ...
        def write_data(payload)
            ...
            @payload = replace_kiss_w("FESC", @payload)
            @payload = replace_kiss_w("FEND", @payload)
            dest_call = SAT_CALL
            dest_ssid = SAT_SSID
            src_call = GST_CALL
            src_ssid = GST_SSID
        ...
        def read_data(data_incoming)
            return super(data_incoming) if (data_incoming.length <= 0)
            data = data_incoming.bytes() #convert data received into bytes format
            #PROCESS DATA
            #KISS PROTOCOL
